...
- [ ] **Step 5: Add a `PR_SMK_DUMP=<dir>` dump path to the test and compare against the capture** — when the variable is set, the test decodes the movie writing **every** decoded frame (no dropping) as `frame_%04d.raw`, **320×200 RGB24 (192000 bytes, converted through the current palette)**. Run `PR_SMK_DUMP=/tmp/port_twg PR_GAME_DIR=data/game/C make test`, then `python3 tools/smk_compare.py --capture data/smk-captures/twg --port /tmp/port_twg --frames 41`. The oracle compares the captured prefix (the frames the original presents); expect an exact match over that prefix, and assert the decoded count equals the header count. If the palette or RGB conversion differs, resolve before proceeding (record the finding).
- [ ] **Step 6: Record the settled presented frame count** — settled by re-capture (commit `199cad1`): the original presents **TWI5 120** of 121 and **TWG 41** of 41. TWI5's last payload frame is a real, distinct image that the original never presents (three independent captures, none containing it). Task 5's oracle compares the captured set (TWI5 0..119, TWG 0..40). The rule that decides presentation belongs to the **player (Task 7)**, not this test: do not encode a content-based heuristic in the test, and do not have the decoder drop frames.
...
- [ ] **Step 5: Run `--check`** — `./build/prageport --game-dir data/game/C --check 120` exits 0 and the headless frame gate still passes; confirm no `mem[0xA0000]` write (the aperture rule) by inspection. Also assert the presented frame count per movie matches the settled capture (TWI5 120, TWG 41). **This player owns the rule that decides presentation** (the decoder decodes all frames): port it from the original's loop (`0x1C740`/`0x6345c`) if the semantics can be read, otherwise implement the documented fixed-profile rule and carry a `/* TODO(verify): */` that the original's player-loop semantics are unproven.
