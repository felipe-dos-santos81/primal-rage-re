...
Cycle commits: 25, `d514f9a..b63e6e4`; the final commit adds this report.
...
The engine can now composite a display list of sprites into the back buffer,
pixel-for-pixel the way `0x14328` does. Three units landed:
...
* `sprite_render_rle` (this cycle) is asserted **byte-identical** to
  ...
  compares the two on **32 real sprites** — every positive-dimension RLE
  descriptor at most 320×200, from the shipped set.
