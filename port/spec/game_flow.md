...
* **The camera/scene layer — `0x1324C` ported, the camera chain now ported too
  ...
  **Ported:** `0x12CD4` (the camera-y stepper, inlined into `camera_y_clamp`),
  ...
  `0x17580`'s `0x140E4`/`0x170A0` tails remain named gaps inside the ported
...
`tools/title_compare.py --frontend` aligns it: window distinct **[560..1714]**
(raw 3108..4621), **1155 frames: 455 clean, 693 splice, 3 transition, 2
unexplained** (the four classes sum to 1153 of 1155: the window's other two frames are the
...
`0x3E4E4` leap) then explained 880..890: → `[560..890]`/331; the frame-891
...
transition frames all lie in that new span; the frame-1358 fix (the camera
split arm's `0x18714` record writes) then explained 1358..1410: →
`[560..1410]`/851; the frame-1411 fix (the T-rex's reaction-`0x20` callback
...
command gate) then explained 1481..1545: → `[560..1545]`/986; the frame-1546
fix (the effects pass's worshipper fall/lie/climb, `0x49C78` cases 3..5) then
explained 1546..1562: → `[560..1562]`/1003; the frame-1563 fix (the effects
pass's per-entry prelude `0x4B69C`, the worshippers' trample) then explained
1563..1658: → `[560..1658]`/1099; the frame-1659 fix (game_frame's fighters'
body push `0x3BB90`) then explained 1659..1714: → `[560..1714]`/1155.)
...
the frame-870 fix 870..879, the frame-880 fix 880..890, the frame-891 fix
891, the frame-892 fix 892..949, the frame-950 fix 950..991 and the
...
* Demo window: distinct **[1881..3616]** (raw **4788..8409**), **1736 frames:
  ...
  **0/2 exhibited**. (Measured on the frame-1763 fix; before it:
  ...
  before the frame-1546 fix:
  `[1546..3616]`, 2071 frames, port `[1092..1380]`, first unexplained 1546;
  before the frame-1481 fix: `[1481..3616]`, 2136 frames, port `[1036..1380]`, first unexplained 1481;
  before the frame-1478 fix: `[1478..3616]`, 2139 frames, port `[1034..1380]`, first unexplained 1478;
  before the frame-1411 fix: `[1411..3616]`, 2206 frames, port `[977..1380]`, first unexplained 1411;
  before the frame-1358 fix: `[1358..3616]`, 2259 frames, port `[931..1380]`, first unexplained 1358;
  before the frame-998 fix: `[998..3616]`,
  2619 frames, port `[623..1380]`, first unexplained 998; before the frame-992
  fix: `[992..3616]`,
  2625 frames, port `[618..1380]`, first unexplained 992; before the frame-950
  fix: `[950..3616]`,
  2667 frames, port `[582..1380]`, first unexplained 950; before the frame-892
  fix: `[892..3616]`,
  2725 frames, port `[532..1380]`, first unexplained 892; before the frame-891
  fix: `[891..3616]`,
  2726 frames, port `[531..1380]`, first unexplained 891; before the frame-880
  fix: `[880..3616]`,
  ...
  fix: `[866..3616]`,
  2751 frames, port `[510..1380]`, first unexplained 866; before the frame-864
  fix: `[864..3616]`,
...
* **First unexplained captured frame 1881 (raw 4788)** (demo-pose record §33.5).
  ...
  1266/1267 splice (row 153) left 5 544 px in x 0–286, rows 153–199: the
  capture's gold T-rex kept port 1266's place and upright pose, while port
  1267 had it further left in another pose.) (Before
  ...
  leap onto the raptor, and the port did not call it. Capture 1659's best
  port 1188/1189 splice (row 135) left 57 px in x 111–124, rows 135–142, and
  1660..1662 left 5 279, 10 954 and 42 868 px.) (Before the frame-1563 fix this was
  capture 1563 (§28.5, derived in §29): at f = 689 the gold fighter's tail
  put both landed side-1 worshippers inside side 0's `0x100AC8` box; the
  effects pass's prelude `0x4B69C` (`0x17D30`/`0x1790C`) tramples them
  through `0x4B470` and case 6 flies them, and the port had no prelude.
  Capture 1563's best port 1106/1107
  splice (row 154) leaves 514 px in x 37–232, rows 158–191, in two places:
  x 200–232, a worshipper beside the right-hand fighter in a different pose,
  and x 37–70 (rows ≈ 180–191), a dark blob under the left fighter; 1566
  leaves 284 px. Port 1106 is the f ≈ 689 state.)
  (Before the frame-1546 fix this was capture 1546 (§27.5, derived in §28):
  a small teal-clad worshipper at x 221–234 crouched in the capture and not
  in the port. The effects pass's case 3 (`0x49DB3`) lands a falling
  worshipper once its y is at or below the word `0xBD898` (`0x400`) on the
  `0xC9634[si]` crouch stream; the port had only the gate and the draw, so
  the two side-1 worshippers scared at f ≈ 650 never crouched at
  f = 675/676.) (Before the frame-1481 fix this
  was capture 1481 (§26.5, derived in §27): the capture's T-rex stood 1 px
  right and its raptor 1 px left of port 1036. At f = 619 the T-rex's command
  word `0x6F6F` has a bit in both `(cmd>>8)&3` and `(cmd>>8)&0xC`, so the
  raw `0x349C8` returns at `0x34A8D`; the port ran its `0x4000` arm, which
  restarted the T-rex's sprite, and the reaction-`0x08` hit's `0x18B04` then
  re-derived its record x from the new sprite, 64 units left.) (Before
  the frame-1478 fix this was capture 1478 (§25.5, derived in §26): capture
  1478's best port 1033/1034 splice (row 103) leaves 7 749 px in rows
  103–195; from 1480 the residual covers the whole frame. Port frame 1034 is
  f = 618. The capture's breath ring reaches the raptor at the left edge and
  bursts as the raptor recoils; the port's ring (slot 0's `+0x08`, −256 per
  frame, x 10 330 at f = 618 against the raptor's record x 10 052) flies on,
  never tested against the raptor. No `fn_resolve` miss falls in f =
  560..624. The candidate owner is the unported collision step `0x17CB0`
  (`0x1975C`'s first call, `0x19763`), whose `0x176CC`/`0x17BC8` test the
  slots' `+0x08` projectiles; it is the owner: the port skipped
  `0x19763 call 0x17CB0`, so `DS_00100AD0` stayed 0 and the think step never
  applied the hit, which it now does at f = 617.) (Before the
  frame-1411 fix this was capture 1411 (§24.5, derived in §25): at f = 559
  the T-rex's reaction `0x20` reached the unregistered callback `0x3D17C`
  (entry `0xA37A8`), so the port put the T-rex back in its stance instead of
  starting the breath stream `0xE84C8`, whose `0x3D214` emitter and
  `0x3D26C` projectile the port now spawns. Before the frame-1358 fix this was capture 1358 (§23.5,
  derived in §24): its best 930/931 splice left 2 743 px in the T-rex at the
  right edge. It read as a late landing, but the capture draws the same
  T-rex sprite 4 px further left from f = 515: the mode-1 camera's split arm
  `0x12E3C` pulls the T-rex back to its `+0x38` latch and rewrites its
  record's `+0x18` through `0x18714`, and the port clamped only the slot.
  Before the frame-998 fix this was capture 998 (§22.5, derived in §23): capture 998's
  best port 622/623 splice (row 107) left 259 px, all in the
  left-edge worshipper (x < 40); 999..1006 leave 481–651 px there: the
  capture's worshipper turns and walks while the port's keeps cheering. Port
  frame 622 is f = 206, the first `fn_resolve` miss after the frame-992 fix:
  a pool record's stream (`rec+8` `0xEE0A0`) reaches the unregistered
  ...
  residual is whole-frame. The capture's background sits 1 px right of the
  port's at 991/992 and 1–2 px left from 994: a horizontal camera and
  fighter-position divergence. Port frame 617 is f = 201, the first
  `fn_resolve` miss after the frame-950 fix: the T-rex's stream reaches the
  unregistered animation-opcode target `0x35938`, which puts the T-rex in
  its state-1 walk; the port now runs it. Before the frame-950 fix this was
  capture 950 (§20.5, derived in
  §21): capture 950 was a tear: the best port 581/582 splice (row 122) leaves
  1 731 px in x 132–264, rows 161–199: the capture's raptor stays lying where
  it landed, and the port's gets up. At f = 165 the raptor's landing stream
  `0xD2ADA` reaches `D500 47B8 0003`, whose opcode-`0x15` target `0x347B8` the
  port has not registered, so it was skipped; with `DS_00104B00` = 3 the raw
  `0x347B8` puts the slot in state 9/`0x0B`/0 and starts a per-character
  ...
  **Named gaps, measured (record §19.6.2):** the `0x1958C` hook `0x19020` →
  ...
  fix's run missed at f = 850/887/896, no longer miss; the reaction-`0x3D` callback `0x3C048`
  (f = 652 on the frame-1478 fix) no longer misses; `0x14814` no longer
  ...
  the struck raptor in the `0x39F40` knockback pose, whose `slot+0x10`
  handler `0x39CC8` the port resolved to NULL, so the raptor was never
  launched and the camera, which follows the higher fighter's y, never
  climbed; record §20 removed that.) (Before the frame-891 fix this was capture 891: the best
  530/531 splice left 2 188 px, the raptor, which the capture shows struck; at
  f = 114 `0x193B0` ran for the T-rex, whose `slot+0x1C` held `0x3E4C4`, and
  the port resolved it to NULL and skipped the `0x3B714` reaction; record
  §19.6 removed that. The first characterisation named the collision step
  and said `0x3E484`/`0x3E4C4` were only called from the think chain; both
  were wrong: their callers are `0x1958C`'s `0x19020` and `0x193B0`.)
  (Before the frame-880 fix this was capture 880: the best
  ...
  gravity 23, speeds 550/−150) and kept the raptor on the ground; record §18
  removed that. Before the frame-867 fix this was capture 867: the best 510/511
  splice (row 124) left 11 184 B / 3 955 px in x 185–319, rows 125–194, the
  raptor's body; `0x3BDDC`'s `0x3C480` call was an unported gap, so the raptor
  held its stance `0x16B5` from f = 94 at state 3/4/2, and `0x18714` omitted
  `0x18540`/`0x18350`, so the new sprite's anchor x never moved `rec+0x18`
  (+7 px); record §17 removed both. Before the
  frame-866 fix this was capture 866: `0x36870`'s case 0 restarted the other
  side's record, so at f = 93 the raptor's `0x36870` opcode put the T-rex on
  the raptor's stance `0xD2136` (`0x96B5`), whose `0x18540` anchor fell to 0 and
  stepped the camera −500 → −256; the best 509/510 splice left 27 858 B /
  9 891 px; the `0x36A8C`/`0x36AA1` operand fix, record §16, removed that.
  Before the frame-864 fix this was capture 864: the grey flier `0x1282C` spawns from
...
frame-870 fix to **880**, the frame-880 fix to **891**, the frame-891
fix to **892**, the frame-892 fix to **950**, the frame-950 fix to
...
`0x3B9D8`; correction, roar-timing Task 20: none of these four was in
`port/src`, and `game_frame` carried the `0x2541D` call as a `PORT:` note;
the frame-1659 fix ports them, demo-pose record §30), the `0x3CF38` hit chain, the `0x3C88C` hitbox machine (§7.7), the
...
### Wrong record in `0x36870` case 0 (`2137bce`), capture 866

* **Cause (raw).** `0x36870`'s `+0x54 == 0` arm restarts the calling side's
  own record: `0x36A8C mov eax,[esp+0x10]` and `0x36AA1 mov edx,[esp+0x10]`
  both read `rec_s` (`0x368E5` stored `[S]` there; `[esp+0x14]`, the other
  record, is stored at `0x368EB` and never read). The port restarted the other
  side's record. At f = 93 the raptor's own stream reaches its `0x36870`
  opcode, so the port put the T-rex on the raptor's stance `0xC8950[3] =
  0xD2136` (`0x16B5`, hflipped `0x96B5`). The T-rex's `0x18540` anchor then
  fell out of range to 0 (`AB0` −448 → 320), which stepped the camera
  −500 → −256.
* **Fix.** Both operands now use `rec_s`. `check_deep_callees` case E is
  mutation-proven: `rec_o` for either operand, the other slot's `+0x7A` and
  `+0x4D = 0x14` each fail.
* **Measured.** Capture 866 is now a 0 B splice of port 509/510 (row 96), from
  27 858 B. The demo's first unexplained is now **867 (raw 3774)**, and the
  fight window `[867..1884]` has 1018 frames, 0 explained. The ratchet N is
  raised 866 → 867.
* **Moved claim (allowed by the brief).** Front-end `[560..865]`/306 →
  **`[560..866]`/307: 137 clean, 166 splice, 0 transition, 2 unexplained
  (832, 833)**. Nothing else moved.

### The raptor's crouch and its anchor (`a76414d`), captures 867..869

* **Cause (raw).** At f = 94 the demo AI's command `0xA0A0` reaches `0x3BDDC`
  for side 1 (the raptor, char 3), which sets `+0x52/+0x53/+0x54` = 3/4/2.
  Before those stores, `0x3BEF7..0x3BF05` call `0x3C480(rec, [0xC8B30 +
  slot+0x7A·4], 1.0f)`: `0xC8B30[3] = 0xD2274`, the crouch `0x1746`,
  `0x1747`, …. The port had the call as a gap, so the raptor held its stance
  `0x16B5`. `0x3C480`'s `0x188DC` tail calls `0x18714`, which runs `0x18540`
  (`0x1873F`) and, when the new anchor differs from `slot+0x20`, `0x18350`
  (`0x1875F`) before returning `slot+0x2C − DS_00100AB0`. The port omitted both
  calls, so the stale anchor x (−4 for `0x16B5`; −11 for `0x1746`, `0xD033B` +
  2·145) left `rec+0x18` at 6144 instead of 6592: the raptor stood 7 px left.
* **Fix.** The `0x3C480` call, and `hit_record_x/y` as the raw has them
  (`0x18788` too, though its only caller `0x1883C` latches first, so it is
  inert there). `check_deep_callees` cases F/G are mutation-proven: the missing
  call, the other slot's `+0x7A` or record, hold 3.0, either anchor call
  dropped, an unconditional `0x18350` and a `slot+0x20` store each fail.
* **Measured.** Captures 867 (510/511 splice), 868 (511/512 splice) and 869
  (port 512) are now 0 B, from 11 184 / 14 505 / 14 034 B. The demo's first
  unexplained is now **870 (raw 3777)**, and the fight window `[870..1884]` has
  1015 frames, 0 explained. The ratchet N is raised 867 → 870.
* **Moved claim (allowed by the brief).** Front-end `[560..866]`/307 →
  **`[560..869]`/310: 138 clean, 168 splice, 0 transition, 2 unexplained
  (832, 833)**. Nothing else moved.

### The raptor's launch (`0x35E04`/`0x3BC70`, `0a8346b`), captures 870..879

* **Cause (raw).** At f = 96 the raptor's attack stream `0xD2274` reaches
  `D000 5E04 0003` (`0xD2278`): opcode `0x10`, mode `0x4000`, the inline
  dword `0x00035E04`. The port registered nothing there, so `anim_indirect`
  skipped it. `0x35E04` (no Ghidra function) stores 3.0f into `rec+0x20` and
  `rec+0x24` when `rec+0x14` is set and calls `0x3BC70(rec+0x51)`, which sets
...
* **Fix.** `fighter_35e04`/`fighter_3bc70`, registered through the
  ...
  registration, the missing call, the wrong side, the gate, the hold, the
  copy, each sign arm, the load order and each slot byte each fail.
* **Measured.** Capture 870 is now port 513 (0 B), and 871..879 are 0 B
  splices (876 is port 518), from 12 025 B at 870. The demo's first
  unexplained is now **880 (raw 3787)**, and the fight window `[880..1884]`
...
* **Fix.** The callback call in `hit_reaction_apply` and the case-7 call now
  ...
  through the `(rec, arg)` wrapper `anim_code_3E4E4`. `0x3E484`/`0x3E4C4` were
  stored, not ported, under the claim that the `0x1975C` think chain was their
  only caller; that was wrong (the next section). `check_trex_leap` is mutation-proven (the missing call, each
...
### The T-rex's `+0x1C` callback (`0x3E4C4`, `6a48972`), capture 891

* **Cause (raw).** `get_function_callers` gives `0x1958C` as the only caller
  of both `0x19020` and `0x193B0`. `0x19020` calls `[slot+0x18]` at
  `0x1903F` with EAX = side and writes `DS_00100AF8[side]` = 1 on a zero
  result, 0 otherwise. `0x193B0` sets the other slot's `+0x90 = 5`, calls
  `[slot+0x1C]` at `0x19505` with EAX = side and zeroes `+0x18`/`+0x1C`. At
  f = 114 the winner body runs for the T-rex with `+0x1C` = `0x3E4C4`, which
  the port resolved to NULL. `0x3E4C4` (31 B) is
  `0x3B714(slot[1-side], slot[side])` through `0x33950`, the `0x19526` call.
* **Fix.** `fighter_3e4c4`, registered at `0x3E4C4`. `check_winner_body`'s
  new `+0x1C` block and the registration check are mutation-proven. The
  unregistered, swapped-argument, `ctx_swap` and empty-body mutations each
...
* **Moved claim (allowed by the brief).** Front-end `[560..890]`/331 →
  **`[560..891]`/332: 142 clean, 186 splice, 0 transition, 2 unexplained
  (832, 833)**. Nothing else moved.

### The knockback pose's handler (`0x39CC8`/`0x39B30`, `a51685d`), captures 892..949

* **Cause (raw).** At f = 114 the reaction `0x3B714` → `0x3AAFC` finds the
  struck raptor airborne (`slot+0x54` = 2) and calls the pose setter `0x39F40`
  at `0x3AC89` (EDX = −80, EBX = `0x46`, ECX = `0x0C`, frame `0x14`), which
  stores `0x39CC8` in `slot+0x10` (`0x39F8F`). `0x3531C` case 10 calls it each
  frame (`0x354E2`); the port resolved it to NULL. `0x39CC8` (561 B, no Ghidra
  function, jump table `0x39CB4`) runs `0x35050` (the slot's `+0x14`
  callback), then a `+0x58` machine: 0 arms; 1 launches through `0x39B30`
  (the `0xBED60[char]` stream at hold 12/7, gravity 62, vertical 744,
  horizontal 160); 2 re-times the fall to the ground (`0x39AC8`, the
  `0xBED88[char]` stream at 20/7); 3 lands (the `0xBEDB0[char]` stream, y to
  `word[0xBECFA + char*2]`, the `0xBB1DC` dust); 4 clears `+0x54`. The camera
  follows the higher fighter's y (`0x12DA8`), so the raw's camera climbs from
  f = 115 and the port's did not: the "scene drops" of capture 892.
* **Fix.** `fighter_39cc8` (registered at `0x39CC8`), `fighter_39b30`,
  `fighter_35050`, `fighter_39ac8`, `fighter_39b14`: 1 152 B in 5 functions.
  `check_knockback_pose` drives every phase and the case-10 wiring with the
  demo raptor's values; 32 of 33 mutations fail it (the `>=` conditional
  anchor is unobservable: at equality `0x1890C` adds 0).
* **Measured.** Captures 892..949 are explained. The demo's first unexplained
  is now **950 (raw 3857)**, and the fight window `[950..1884]` has 935
  frames, 0 explained. The ratchet N is raised 892 → 950.
* **Moved claim (allowed by the brief).** Front-end `[560..891]`/332 →
  **`[560..949]`/390: 150 clean, 236 splice, 0 transition, 2 unexplained
  (832, 833)**. Nothing else moved.

### The knockdown floor (`0x347B8`/`0x346F8`, `3fee8d0`), captures 950..991

* **Cause (raw).** At f = 165 the landed raptor's stream `0xD2ADA` reaches
  `D500 47B8 0003` at `0xD2B00`; the port's `fn_resolve(0x347B8)` returned
  NULL, so the dispatcher walked on into the ids that follow and the raptor
  got up. `0x347B8` (448 B, no Ghidra function) runs `0x39A10(rec, 0x29A)`,
  `0x3C148`, `0x3C16C`, sets state 9/`0x0B`/0 and `+0x43 &= 0xFC`, then the
  stun gate `0x340BC` (`0x3485F`) picks the stream: on 1 the stun start
  `0x34168` and `0x34780[char]`, on 0 `0x3479C[char]`, at hold 3.0 (game mode
  7 may freeze the side instead, `0x3480F`). The demo raptor fails the gate
  (`+0x63` = 1, `+0x5A` 22 against 9) and lies on `0xD28FC`, a 20-pass loop
  ending in the get-up `0x346F8` (`+0x76` = `word[0xBDBE6]` + 1, then
  `0x36870`) at f = 206.
* **Fix.** `fighter_347b8`, `fighter_340bc`, `fighter_34168`,
  `fighter_346f8` (1 029 B in 4 functions), registered through `(rec, arg)`
  wrappers. The slot `+0x14` callback now takes the slot at its three raw
  call sites. `check_knockdown_floor` drives the gate's bounds, the demo's
  floor through the dispatcher, both stream tables, the stun arm on both
  sides, mode 7 and the get-up; every mutation fails it but the pushed hold
  3.0, which every floor stream's `DC00` replaces (unobservable).
* **Measured.** Captures 950..991 are explained. The demo's first unexplained
  is now **992 (raw 3899)**, and the fight window `[992..1884]` has 893
  frames, 0 explained. The ratchet N is raised 950 → 992.
* **Moved claim (allowed by the brief).** Front-end `[560..949]`/390 →
  **`[560..991]`/432: 170 clean, 258 splice, 0 transition, 2 unexplained
  (832, 833)**. Nothing else moved.

### The walk entry (`0x35938`, `ff38dcc`), captures 992..997

* **Cause (raw).** At f = 201 the T-rex's stream (state `0x0E`) reaches
  `D500 5938 0003` at `0xE6EE8`; the port's `fn_resolve(0x35938)` returned
  NULL, so the dispatcher walked on into the ids that follow and
  `D500 7068 0003` (`0x36870`, state 0), a loop that kept the T-rex in place
  while the capture's walked and the camera followed. `0x35938` (166 B, no
  Ghidra function, 14 data sites) sets state 1/0 (8 when `+0x54` is 4,
  `+0x53` kept), sets `rec+0x29` bit 3 and seeks `rec+8` through `0x2BCF4`
  to the literal id of `0x35C1C`'s `+0x43`-selected frame table
  (`0xC8AE0`/`0xC8A68`) at the record's signed frame `rec+0x52`, then
  `rec+0x52` = 0, `rec+0x20` = `rec+0x24` = 0, `rec+0x58` = 1 and
  `rec+0x28 |= 0x804`. The state-1 handler `0x359E0` walks it from f = 202.
* **Fix.** `fighter_35938`, registered through a `(rec, arg)` wrapper.
  `check_walk_entry` drives the demo's entry, the signed index, both tables,
  side 1, the state-8 arm, the no-slot return and the dispatcher; every
  mutation fails it.
* **Measured.** Captures 992..997 are explained. The demo's first unexplained
  is now **998 (raw 3905)**, and the fight window `[998..1884]` has 887
  frames, 0 explained. The ratchet N is raised 992 → 998.
* **Moved claim (allowed by the brief).** Front-end `[560..991]`/432 →
  **`[560..997]`/438: 172 clean, 262 splice, 0 transition, 2 unexplained
  (832, 833)**. Nothing else moved.

### The worshipper arrival target (`0x4AC18`, `b5a48a0`), captures 998..1357

* **Cause (raw).** At f = 206 the left-edge worshipper's cheer stream (its
  fight-effect entry `0x1083CC` in type 8) reaches `D500 AC18 0004` at
  `0xEE09C`; the port's `fn_resolve(0x4AC18)` returned NULL, so the
  dispatcher walked on into the next cheer stream at `0xEE0A2` and the
  worshipper kept cheering while the capture's turned and walked.
  `0x4AC18` (29 B, no Ghidra function, 24 data sites in
  `0xEE09E..0xEF62E`) calls the already-ported arrival
  `0x4AC38(rec+0x14 entry, (u32)(u8)rec+0x48 − 0x20)`: the entry returns to
  type 0 and its actor begins the `0xC9544[index]` stream at the hold 5.0;
  the entry's type-0 handler `0x4AAD0` then walks it.
* **Fix.** `fight_4ac18`, registered through a `(rec, arg)` wrapper.
  `check_worshipper_arrival` drives the demo's call, the index (its `− 0x20`,
...
* **Moved claim (allowed by the brief).** Front-end `[560..997]`/438 →
  **`[560..1357]`/798: 307 clean, 484 splice, 3 transition, 2 unexplained
  (832, 833)**; the three transition frames all lie in the new span, and
  port frames 0..622 are byte-identical to before. Nothing else moved.

### The camera split arm's `0x18714` writes (`c78dc97`), captures 1358..1410

* **Cause (raw).** At the end of f = 514 and f = 515 the fighters' `+0x34`
  latches are 20 526 apart (T-rex 26 674, raptor 6 148), more than
  `word[0x9AF28]` = `0x5000`. The mode-1 camera `0x12E3C` pulls the
  outward-moving T-rex (slot b, `+0x34` > `+0x38`) back to its `+0x38` latch,
  the previous frame's `+0x34`: it stores `+0x34` and `+0x2C`, then rewrites
  the T-rex record's `+0x18` through `0x18714` (`0x12F22 call`,
  `0x12F29 mov [edx+0x18],eax`; slot a's twin at `0x12F02`/`0x12F09`). The
  port stored only the slot fields, so the next latch rebuilt `+0x2C` from
  the unclamped record and the T-rex ran two frames' worth of x (436)
  further right. The capture draws the same sprite 4 px further left; it is
  not a late landing, as §23.5 read it.
* **Fix.** Both arms now call `hit_record_x` (`0x18714`, exported from
  `fighter.c`) after their slot stores, in the raw's order.
  `check_camera_split` drives the demo's slot-b clamp, the slot-a clamp with
  its own side, the `jle` gate at exactly `0x5000` and the `+0x42` bit-3
  arm; every mutation fails it.
* **Measured.** Captures 1358..1410 are explained; port frames 0..930 are
  byte-identical to before. The demo's first unexplained is now **1411 (raw
  4318)**, and the fight window `[1411..1884]` has 474 frames, 0 explained.
  The ratchet N is raised 1358 → 1411.
* **Moved claim (allowed by the brief).** Front-end `[560..1357]`/798 →
  **`[560..1410]`/851: 337 clean, 507 splice, 3 transition, 2 unexplained
  (832, 833)**; the three transition frames are the same as before. Nothing
  else moved.

### The T-rex's reaction-`0x20` breath (`4065c1d`), captures 1411..1477

* **Cause (raw).** At f = 559 `0x34E2C` applies reaction `0x20` to the T-rex;
  its `(char 0, 0x20)` entry `0xA37A8` holds only the callback `0x3D17C`,
  which the port skipped as unregistered. `0x3D17C` returns at `0x3D187` while
  the slot's `+0x08` holds a live projectile; otherwise it starts the stream
  `0xE84C8` at hold 3.0 through `0x3C4CC` (EDX = `0xE84C8`, loaded for the
  `0x2C3FC` voice at `0x3D198`, survives it), sets state `0xB/6/0`, clears
  `+0x0C/+0x18/+0x1C`, moves `+0x5F` to `+0x64` and stores `0x100` in the word
  `0x1080AC[rec+0x51]` (`0x3D1E0`). The stream's `D100 D214 0003` spawns the
  emitter `0xBB27C` as the record's child (`0x3D214`); the emitter stream's
  `D100 D26C 0003` spawns the projectile `0xBB268` beside the record into the
  slot's `+0x08` at `∓0x1080AC[side]` (`0x3D26C`); the projectile's type-2
  teardown `0x3B9C4` clears `+0x08` again.
* **Fix.** `fighter_3d17c`/`fighter_3d214`/`fighter_3d26c` in `fighter.c`,
  registered in `actors_init` (the two opcode targets through `(rec, arg)`
  wrappers). `check_trex_breath` covers the demo's f = 559 dispatch, the
  live-projectile return, the `rec+0x51` index, both spawns per side and
  flip, and the dispatched `0x3D214`; every mutation fails it. The two
  existing reaction-`0x20` checks now seed a live projectile, so their
  assertions are unchanged.
* **Measured.** Captures 1411..1477 are explained; port frames 0..975 are
  byte-identical to before. The demo's first unexplained is now **1478 (raw
  4385)**, and the fight window `[1478..1884]` has 407 frames, 0 explained.
  The ratchet N is raised 1411 → 1478.
* **Moved claim (allowed by the brief).** Front-end `[560..1410]`/851 →
  **`[560..1477]`/918: 382 clean, 529 splice, 3 transition, 2 unexplained
  (832, 833)**; the three transition frames are the same as before. Nothing
  else moved.

**The projectile collision step (`3d64c61`, demo-pose record §26).**
* **Cause.** `0x1975C`'s first call `0x19763 call 0x17CB0` was unported, so
  `DS_00100AD0/AD4` stayed 0 and the think step never ran: at f = 617 the
  T-rex's breath ring flew through the raptor.
* **Raw.** `0x17CB0` fills the 0x25-byte `0x100BD3` plane with `0xFF`
  (`0x15F48`), zeroes `AD0/AD4`, then tests two live projectiles (`0x17BC8`:
  `0x3B938` bursts side 0's, `0x2B150` kills side 1's) or each side's
  projectile against the other fighter (`0x176CC`: the `0x170A0` shape, the
  projectile a `0x20` box at `DS_00100AA8/AA0[side]`, `0x16DA4` mode 0 with
  the row `0xA173C`), writing `AD0[side]` = `DS_00100B54`. `0x1975C` then,
  per thrower with `AD0` > 2 (signed), runs `0x1922C`, the `0x3962C`/`0x396AC`
  gates (true: `0x18B44` and stop), `0x3B464` for the struck side (block:
  `0x3B080`/`0x3AD98`; else `0x2BD44`, the `+0x14` callback, the projectile's
  `+0x48` switch, `0x39834` and `0x39F40` or the `0x3A95C` stagger with
...
* **Measured.** Captures 1478..1480 are explained; port frames 0..1033 are
  byte-identical to before. The demo's first unexplained is now **1481 (raw
  4388)**, and the fight window `[1481..1884]` has 404 frames, 0 explained.
  The ratchet N is raised 1478 → 1481.
* **Moved claim (allowed by the brief).** Front-end `[560..1477]`/918 →
  **`[560..1480]`/921: 383 clean, 531 splice, 3 transition, 2 unexplained
  (832, 833)**; the three transition frames are the same as before. Nothing
  else moved.

**The `0x349C8` command gate (`219691e`, demo-pose record §27).**
* **Cause.** The port's `0x349C8` (`+0x52` == 0) skipped only the `0x3BDDC`
  consume for a command word with a bit in both `(cmd>>8)&3` and
  `(cmd>>8)&0xC`, and still ran the `0x4000` arm and `0x35838`. At f = 619 the
  T-rex's word `0x6F6F` restarted its record on `0xC8978[0]` (sprite `0x0F02`
  → `0x0F34`, `+0x52/+0x54` = 5/1); the reaction-`0x08` hit's `0x18B04` then
  re-derived its record x from the new sprite's anchor (`DS_00100AB0[0]` −320
  → −256): 64 units, 1 px, left of the raw's.
* **Raw.** `0x34A8D 0F 85 78 00 00 00  jne 0x34B0B` returns before the
  `0x3BDDC` call (`0x34A9F`), the `0x4000` arm (`0x34AAB`) and `0x35838`
  (`0x34B06`). `0x350D0`'s own gate (`0x35209 75 1C`) skips only the consume.
* **Fix.** `fighter_state_default` returns on that gate. New case I in
  `check_deep_callees` (`0x6F6F` keeps every sentinel; `0x4040` runs the
  `0x4000` arm); removing the gate and an unconditional return both fail it.
* **Measured.** Captures 1481..1545 are explained; port frames 0..1035 are
  byte-identical to before. The demo's first unexplained is now **1546 (raw
  4453)**, and the fight window `[1546..1884]` has 339 frames, 0 explained.
  The ratchet N is raised 1481 → 1546.
* **Moved claim (allowed by the brief).** Front-end `[560..1480]`/921 →
  **`[560..1545]`/986: 410 clean, 569 splice, 3 transition, 2 unexplained
  (832, 833)**; the three transition frames are the same as before. Nothing
  else moved.

**The worshippers' fall, lie and climb (`27c95c0`, demo-pose record §28).**
* **Cause.** The effects pass `0x49C78`'s type-3 handler `0x49DB3` was ported
  only as its `DS_000BD898` gate and its `rng(0x3C)` draw. The raw sets the
  actor's `+0x2C` from its y (`0x496AC`) every frame and, once the y is at or
  below the zero-extended word `0xBD898` (`0x400`), lands it; the port kept
  the entry at type 3 on its upright stream and drew `rng(0x3C)` every later
  frame. The two side-1 worshippers scared at f ≈ 650 (`0x4B5A8`) land at
  f = 675/676 in the raw.
* **Raw.** `0x49DD8 0F 8F 8A 06 00 00  jg 0x4a468` (the gate, against
  `0x49DCD 66 A1 98 D8 0B 00`, zero-extended); then hflip OR-ed when
  `0x2BE1C` > 0 (`0x49E0B`), `0x2BC30(0xC9634[si], 2.0)` (`0x49E24`),
  `+0x38`/`+0x34` = 0, the `+0x18` timer = `rng(0x3C) + 0x3C` (`0x49E3A`),
  `+0x1C` |= `0x80`, type 4. Case 4 (`0x49E5A`) counts the timer down (signed)
  and starts the `0xC95EC[si]` rise (type 5); case 5 (`0x49EC0`) climbs back
  to `+0x1A` and returns to type 0 on `0xC9544[si]`. `0x496AC` is signed
  (`0x496AD`/`0x496BB`).
* **Fix.** Cases 3..5 in `fight_effects_pass`; `fight_dust_clamp` signed. New
  `check_effects_fall`; 31 mutations each fail it.
* **Measured.** Captures 1546..1562 are explained; port frames 0..1091 are
  byte-identical to before. The demo's first unexplained is now **1563 (raw
  4470)**, and the fight window `[1563..1884]` has 322 frames, 0 explained.
  The ratchet N is raised 1546 → 1563.
* **Moved claim (allowed by the brief).** Front-end `[560..1545]`/986 →
  **`[560..1562]`/1003: 413 clean, 583 splice, 3 transition, 2 unexplained
  (832, 833)**; the three transition frames are the same as before. Nothing
  else moved.

**The worshippers' trample (`cc38a38`, demo-pose record §29).**
* **Cause.** `0x49C78` calls its per-entry prelude `0x4B69C(entry, si)` before
  the type dispatch (`0x49CFE`); the port did not. At f = 689 the gold
  fighter's tail puts both landed side-1 worshippers (type 4, `+0x1C` bit 7)
  inside side 0's `0x100AC8` box, so the raw tramples them and the port left
  them lying.
* **Raw.** `0x4B69C` returns unless `+0x1C` bit 7; tests the actor's pset
  words `+4`/`+8` through `0x17D30` (x/64 − `0x18`, y/64 − `0x38`; side 0 then
  side 1 via `0x1790C`, the `0x176CC` shape with a `0x30` x `0x38` point box
  and the `0xA1740` overlap row); both sides count as side 0; `0x4B788`
  returns 1 (trample) when `0x105B3A` > 1, the other slot's `+0x54` is 3 or the
  side's slot `+0x5F` is not its grab move `0xC97F2[ch]`; then `+0x20` = the
  side, `+0x1F` counts, a held actor (`+0x4A`) is released, and `0x4B470`
  starts `0xC9604[si]` at 3.0, spawns a shadow from `0xBB920[si]`, sets
  `+0x34` = ±`0x80` and `+0x36` = `0x240`, type 6. Case 6 (`0x49F11`) sets bit
...
* **Measured.** Captures 1563..1658 are explained; port frames 0..1105 are
  ...
  4566)**, and the fight window `[1659..1884]` has 226 frames, 0 explained.
  The ratchet N is raised 1563 → 1659.
* **Moved claim (allowed by the brief).** Front-end `[560..1562]`/1003 →
  **`[560..1658]`/1099: 447 clean, 645 splice, 3 transition, 2 unexplained
  (832, 833)**; the three transition frames are the same as before. Nothing
  else moved.

**The fighters' body push (`1268371`, demo-pose record §30).**
* **Cause.** game_frame's `DS_00104B15` tail calls `0x3BB90` at `0x2541D`,
  before `0x12D48`; the port did not. The fighters first overlap at f = 772,
  the T-rex's leap onto the raptor, so the raw pushes them apart (capture
  1659: the T-rex's claw 1 px left and the raptor 1 px right in the port) and
  the port let them overlap.
* **Raw.** `0x3BB90` latches both slots (`0x186D0`), returns when either
  slot's `+0x42` bit 2 is set, copies the slots' `+0x2C`/`+0x30` to
  `0xD3388..0xD3394`, sums the `0xBEEF8` widths (`0x800, 0x700, 0x900,
  0x700, 0x900, 0x580, 0x600`, halved for `+0x54` = 2) into `0xD33A8`, and
  runs `0x3BAEC(sum − d)` when `0x4FB20` returns a distance d: 0 when |dx|
  or |dy| exceeds the sum, else max + min/4 + min/8 when that is below it.
  `0x3BAEC` runs `0x3B9D8` for side 0 then 1 (side 1 first when only slot
  0's `+0x43` bit 1 is set); `0x3B9D8` sets both `+0x41` bit 7, moves the
  side by the other slot's word `+0x4C` when that slot's `+0x43` bit 1 is
  set (raising `DS_00107D30`), else half the penetration away from the
  other through `0x1883C` (the other side instead at the `0x7C00` wall,
  `0x3B8D8`), and with `+0x54` = 2 zeroes a record speed that does not
  point away.
...
**The reaction callbacks `0x3C0A4`/`0x3BF70` (`c7320b2`, demo-pose record §32).**
* **Cause.** `0x34E2C`'s `(char, reaction)` table holds `0x3C0A4` as
  reaction `0x3E`'s callback and `0x3BF70` as reaction `0x3F`'s (the
  T-rex's records `0xA3A00`/`0xA3A14`, one pair per character at stride
  `0x500`; no code reference to `0x3C0A4`). The port had not registered
  `0x3C0A4`, so the `0x35045` call skipped it. At f = 850 the T-rex takes
  reaction `0x3E` (capture 1750: the T-rex's place and pose).
* **Raw.** EAX = slot, EDX = rec, EBX = side. `0x3BF70` returns 0 when the
  side's slot has `+0x40` bit 7. Otherwise it clears the side's record's
  `+0x34`/`+0x43`/`+0x42`, sets `+0x5F` = `0xFF`, stores the `0xBEFA0 + 6 *
  char` row in `DS_00107D40 + side*4`, begins the EDX record on
  `0xC8B30[slot+0x7A]` at hold 2.0 through `0x3C4CC`, writes state 3/4/2,
  `+0x40 |= 0x80` and `DS_001078F8 + side` = 1, and sets `+0x4E` to
  `0xFFFF` when `0x1A570(side)` is non-zero, else 1. It returns 1.
  `0x3C0A4` calls it and, on success, sets `+0x4E` the other way.
* **Fix.** `fighter_3bf70`/`fighter_3c0a4` in `fighter.c`; the
  `(slot, rec, side)` wrappers and both `fn_register`s in `actors.c`. New
  `check_reaction_attack`; 36 of 37 mutations fail it, and the 37th (the
  wrapper passing the slot's own record) is equivalent at the only call
  site.
* **Measured.** Captures 1750..1762 are explained; port frames 0..1266 are
  byte-identical to before. The demo's first unexplained is now **1763 (raw
  4670)**, and the fight window `[1763..1884]` has 122 frames, 0 explained.
